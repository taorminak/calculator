...
describe('Calculator', () => {
  ...
  it('resets second operand to empty strings after calculation', async () => {
    ...
    fetch.mockResolvedValueOnce(createFetchResponse(response));
  ...
  it('calculates the result and updates values after the third operator is applied', async () => {
    ...
    fetch.mockResolvedValueOnce(createFetchResponse(response1));
    await wrapper.vm.calculateResult();
    await nextTick();
    ...
    fetch.mockResolvedValueOnce(createFetchResponse(response2));
    await wrapper.vm.calculateResult();
    await nextTick();
  ...
  it('throws an error when passing unsupported operator', async () => {
    wrapper.setData({
      firstOperand: '1',
      secondOperand: '5',
      operator: '#'
  ...
  it('throws an error when fetch calculation result fails', async () => {
    fetch.mockRejectedValueOnce(new Error('Failed to fetch'));
  
    await expect(() => wrapper.vm.fetchCalculationResult()).rejects.toThrowError();
  });
});
