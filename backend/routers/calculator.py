...
def multiplicate(request: OperationRequest, calculator_service: CalculatorService = Depends(get_calculator_service)):
    result = calculator_service.multiplicate(request.operand1, request.operand2)
    return {"result": result}

@router.post("/divide")
def divide(request: OperationRequest, calculator_service: CalculatorService = Depends(get_calculator_service)):
    result = calculator_service.divide(request.operand1, request.operand2)
    return {"result": result}

@router.post("/modulo")
def modulo(request: OperationRequest, calculator_service: CalculatorService = Depends(get_calculator_service)):
    result = calculator_service.modulo(request.operand1, request.operand2)
    return {"result": result}

@router.post("/exponentiate")
def exponentiate(request: OperationRequest, calculator_service: CalculatorService = Depends(get_calculator_service)):
